...
import (
	...
	"github.com/elizcarvalho/fclx/chatservice/internal/domain/gateway"
...
func NewChatCompletionUseCase(chatGateway gateway.ChatGateway, openAiCliente *openai.Client, stream chan ChatCompletionOutputDTO) *ChatCompletionUseCase {
	return &ChatCompletionUseCase{
		ChatGateway:  chatGateway,
		OpenAiClient: openAiCliente,
		Stream:       stream,
	}
}
...
func (uc *ChatCompletionUseCase) Execute(ctx context.Context, input ChatCompletionInputDTO) (*ChatCompletionOutputDTO, error) {
	...
	userMessage, err := entity.NemMessage("user", input.UserMessage, chat.Config.Model)
	if err != nil {
	...
	assistant, err := entity.NemMessage("assistant", fullResponse.String(), chat.Config.Model)
	if err != nil {
...
func CreateNewChat(input ChatCompletionInputDTO) (*entity.Chat, error) {
	...
	initialMessage, err := entity.NemMessage("system", input.Config.InitialSystemMessage, model)
	if err != nil {
